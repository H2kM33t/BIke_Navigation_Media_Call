...

{
...

    {
        ...
        Serial.printf("Nav RX (%u bytes): ", (unsigned)val.length());
        ...
        // Forward raw bytes to navigation.h parser and show nav screen immediately.
        updateNavFromBLE((const uint8_t *)val.data(), val.length());
...

{
    ...
    navSvc->start(); // FIX: must start nav service before pServer->start()
